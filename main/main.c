...
#define WIFI_SSID "Joao_rocha"
#define WIFI_PASSWORD "12345567"
...
void gpio_callback(uint gpio, uint32_t events) {
    if (gpio == botao) {
        button_pressed = (events & GPIO_IRQ_EDGE_FALL) != 0;
    }
}
...
static void init_button() {
    ...
    gpio_pull_up(botao);
    gpio_set_irq_enabled_with_callback(botao, GPIO_IRQ_EDGE_FALL | GPIO_IRQ_EDGE_RISE, true, &gpio_callback);
}
...
void wifi_task(void *p) {
    ...
    while (1) {
        ...
        payload_length = sprintf(payload_content, "dado=%d", button_pressed);
